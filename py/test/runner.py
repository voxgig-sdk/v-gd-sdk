...
class VGdTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VGdTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VGdTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VGdTestRunner.getenv("VGD_TEST_LIVE")
        override = VGdTestRunner.getenv("VGD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VGdTestRunner.getenv(key)
        ...
        explain = VGdTestRunner.getenv("VGD_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VGD_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if VGdTestRunner._test_control is not None:
            return VGdTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VGdTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VGdTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VGdTestRunner.load_test_control()
...
def load_env_local():
    VGdTestRunner.load_env_local()


def env_override(m):
    return VGdTestRunner.env_override(m)


def entity_list_to_data(lst):
    return VGdTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VGdTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VGdTestRunner.load_test_control()


def live_delay_ms():
    return VGdTestRunner.live_delay_ms()
