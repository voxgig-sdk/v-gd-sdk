...
AQL-driven command-line client **and** interactive REPL for the VGd
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/v-gd-cli)
make build
...
./v-gd-cli load 1 url_shortening            # {id:1} shorthand
./v-gd-cli load '{id:1}' url_shortening       # explicit match map

# 4. Override the API base URL for a single call
V_GD_BASE=https://api.example.com ./v-gd-cli load 1 url_shortening

# 5. No arguments -> interactive REPL
./v-gd-cli
v-gd> load 1 url_shortening
v-gd> :quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/v-gd-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/v-gd-cli load 1 url_shortening
   ```
...
```sh
./v-gd-cli load 1 url_shortening          # scalar shorthand for {id:1}
./v-gd-cli load '{id:1}' url_shortening     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `v-gd>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/v-gd-cli`. |
...
(`./v-gd-cli <expr>`) and interactively (the REPL), and expressions compose the
