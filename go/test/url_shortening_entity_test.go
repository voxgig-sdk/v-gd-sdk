...
func TestUrlShorteningEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set VGD_TEST_URL_SHORTENING_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func url_shorteningBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VGD_TEST_URL_SHORTENING_ENTID")
	...
	if env["VGD_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewVGdSDK(core.ToMapAny(mergedOpts))
	}
