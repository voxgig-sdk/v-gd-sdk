...
class UrlShorteningEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VGD_TEST_URL_SHORTENING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function url_shortening_basic_setup($extra)
{
    ...
    $client = VGdSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("VGD_TEST_URL_SHORTENING_ENTID");
    ...
    if ($env["VGD_TEST_LIVE"] === "TRUE") {
        ...
        $client = new VGdSDK(Helpers::to_map($merged_opts));
    }
