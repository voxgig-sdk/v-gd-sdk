...
class FtCtrl extends VGdControl
{
...
class FtClient
{
    ...
    private VGdUtility $utility;

    public function __construct(VGdUtility $utility, array $options)
    {
    ...
    public function get_utility(): VGdUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (VGdContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new VGdUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (VGdContext $_ctx, string $_url, array $fetchdef): array {
    ...
    private static function buildUrl(VGdSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof VGdError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new VGdResponse($fetched);
        }
    ...
    private function fail(VGdContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (VGdContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
