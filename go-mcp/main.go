// Command VGd-mcp is an MCP server exposing the VGd
// SDK as MCP tools.
//
// Two tools are registered:
//
//	VGd_list  — list records of an entity
//	VGd_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: V_GD_APIKEY carries the API key and
	// V_GD_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("V_GD_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("V_GD_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewVGdSDK(opts)
