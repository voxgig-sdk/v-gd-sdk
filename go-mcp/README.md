...
[MCP](https://modelcontextprotocol.io) server exposing the VGd SDK as
two agent tools — `v-gd_list` and `v-gd_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/v-gd-mcp)
make build

# 2. Provide credentials via the environment
export V_GD_APIKEY=sk_live_xxx
...
```jsonc
// v-gd_list: first page of records
{ "entity": "url_shortening" }
{ "entity": "url_shortening", "query": { } }

// v-gd_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/v-gd-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user v-gd \
     -- "$PWD"/dist/*/v-gd-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `v-gd_list` and `v-gd_load` tools now appear
   in new sessions. Ask the agent to *"list url_shortening using v-gd"*
   and it calls `v-gd_list` with `{"entity":"url_shortening"}`.
...
```sh
export V_GD_APIKEY=sk_live_xxx            # API key
export V_GD_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `v-gd_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `v-gd_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `V_GD_APIKEY` | API key sent with every request. |
| `V_GD_BASE` | Optional override of the API base URL. |
