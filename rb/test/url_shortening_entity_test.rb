...
class UrlShorteningEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set VGD_TEST_URL_SHORTENING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def url_shortening_basic_setup(extra)
  ...
  entid_env_raw = ENV["VGD_TEST_URL_SHORTENING_ENTID"]
  ...
  if env["VGD_TEST_LIVE"] == "TRUE"
    ...
    client = VGdSDK.new(Helpers.to_map(merged_opts))
  end
