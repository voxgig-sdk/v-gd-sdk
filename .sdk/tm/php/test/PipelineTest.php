...
{
    private static function utility(): VGdUtility
    {
        return new VGdUtility();
    }
    ...
    private static function ctx(array $over = []): VGdContext
    {
        ...
        $ctx->op = new VGdOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof VGdError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new VGdOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = VGdMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new VGdOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = VGdMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new VGdOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = VGdMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VGdSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = VGdMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new VGdResponse([]),
            'result' => new VGdResult([]),
        ]);
        [, $err] = VGdMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new VGdSpec(['step' => 's']),
            'response' => new VGdResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new VGdResult([]),
        ]);
        [, $err] = VGdMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new VGdSpec(['step' => 's']),
            'response' => new VGdResponse(self::resp(200, ['v' => 1])),
            'result' => new VGdResult([]),
        ]);
        [, $err] = VGdMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new VGdSpec(['step' => 's']),
            'response' => new VGdResponse(self::resp(200, ['v' => 2])),
            'result' => new VGdResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        VGdMakeResponse::call($ctx);
    ...
    {
        $preset = new VGdResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new VGdSpec([]),
            'response' => new VGdResponse([]),
            'result' => new VGdResult([]),
        ...
        [$got, $err] = VGdMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new VGdResult([])]);
        [, $err] = VGdMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new VGdSpec([]), 'result' => null]);
        [, $err] = VGdMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VGdSpec(['step' => 's']),
            'result' => new VGdResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new VGdOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VGdMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VGdSpec(['step' => 's']),
            'result' => new VGdResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new VGdOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VGdMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VGdResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new VGdSpec([]),
            'result' => new VGdResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = VGdMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new VGdError('boom', 'boom');
        $utility->fetcher = function (VGdContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VGdSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = VGdMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (VGdContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = VGdMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(VGdResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (VGdContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VGdSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = VGdMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new VGdResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new VGdSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = VGdMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = VGdMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new VGdResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, VGdDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new VGdResult(['ok' => false])]);
        $this->expectException(VGdError::class);
        VGdDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new VGdResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', VGdMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new VGdBaseFeature();
        $b = new VGdBaseFeature();
        VGdFeatureAdd::call($ctx, $a);
        VGdFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): VGdBaseFeature
    {
        $f = new VGdBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): VGdContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new VGdSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new VGdResponse(['headers' => null]),
            'result' => new VGdResult([]),
        ]);
        VGdResultHeaders::call($ctx);
