...
//     (\`new VGdSDK(...)\` / \`VGdSDK.test(...)\` become
//     \`VGdSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new VGdSDK(...)\` and any
// \`VGdSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+VGdSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/VGdSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
